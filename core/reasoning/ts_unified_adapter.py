...
try:
    ...
    from reasoner.ts_reasoner.typed_support import TypedSupportObject as _real_typed
...
def make_typed_support(support_type, channel, premises, claim, passed):
    if HAS_TS_REASONER:
        return _real_typed(
            ...
            claim,
            passed,
            trace_hash=canonical_hash(premises),
        )
...
def verify_and_emerge(graph, facts, query_subj, query_pred):
    ...
    proof = run_proof_verification(
        [], query_subj, query_pred
    )  # caller prepares relations
    support = make_typed_support(
        "transitive",
        "transitive_all",
        tuple(facts),
        f"{query_subj} {query_pred}",
        bool(proof),
    )
    return {
        "proof": proof,
        "support": support.to_dict() if hasattr(support, "to_dict") else support,
    }
