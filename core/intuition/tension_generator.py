...
import os
bozo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'bozo'))
sys.path.insert(0, bozo_path)

try:
    from bozo.model import TensionLM, TensionConfig, generate as tension_generate
...
class TensionGenerator:
    def __init__(self, checkpoint_path: str = None):
        ...
        print(f'TensionGenerator loaded 117M model for TS intuition/generation')

    def generate_from_context(self, context_text: str, max_new: int = 50, temp: float = 0.7) -> str:
    ...
    def propose_from_graph(self, graph_state: dict, query: str = "", max_new: int = 40) -> str:
        ...
        if self.model is None:
            contents = [n.get('content', '') for n in graph_state.get('high_activation', [])]
            base = 'Based on verified graph state: ' + ' '.join(contents[:3]) + '.'
            if query:
                base = query.strip() + ' ' + base
            return base
        contents = [n.get('content', '') for n in graph_state.get('high_activation', [])]
        context = ' '.join(contents) if contents else 'Reasoning from verified TS state.'
        if query:
