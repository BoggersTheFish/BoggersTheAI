...
def evaluate_cases(
    model: TinyCandidateModel, cases: list[dict[str, Any]]
) -> dict[str, Any]:
    rows = [evaluate_case(model, case) for case in cases]
    return {
        "case_count": len(rows),
        "metrics": {
            "candidate_ranking_accuracy": rate(
                [row["candidate_ranking_correct"] for row in rows]
            ),
            ...
            "verifier_beats_model_confidence_rate": rate(
                [
                    value
                    for row in rows
                    for value in row["verifier_beats_confidence_checks"]
                ]
            ),
            ...
            "deeper_chain_success_rate": rate(
                [value for row in rows for value in row["deeper_chain_checks"]]
            ),
            "distractor_robustness": rate(
                [
                    row["candidate_ranking_correct"]
                    for row in rows
...
def evaluate_case(model: TinyCandidateModel, case: dict[str, Any]) -> dict[str, Any]:
    ...
    top_candidate_id = (
        payload["scored_candidates"][0]["candidate_id"]
        if payload["scored_candidates"]
        else ""
    )
    candidate_ranking_correct = not accepted_ids or top_candidate_id in accepted_ids
    accepted_support_checks = [
        bool(
            {"logic_transitivity", "surface_structure"}
            & set(by_id[candidate_id]["channels"])
        )
    ...
    accepted_confidences = [
        float(
            scored_by_id[candidate_id].get(
                "confidence", by_id[candidate_id]["confidence"]
            )
        )
        for candidate_id in accepted_ids
    ...
    if accepted_confidences:
        ...
        for candidate_id, label in labels.items():
            if (
                ...
                and candidate_id in scored_by_id
            ):
                result = by_id[candidate_id]
                candidate_confidence = float(
                    scored_by_id[candidate_id].get("confidence", result["confidence"])
                )
                if candidate_confidence > accepted_min:
                    verifier_beats_confidence_checks.append(
    ...
    for candidate_id, label in labels.items():
        ...
        for channel in CHANNEL_LABELS:
            channel_activation_checks.append(
                (channel in predicted_channels) == (channel in actual_channels)
            )
    ...
    contamination_count = sum(
        ...
        for status in result.get("typed_runtime", {})
        .get("context", {})
        .get("surface_tags", {})
        .values()
        if status == "candidate"
...
def trace_schema_valid(payload: dict[str, Any]) -> bool:
    ...
    return all(
        key in payload for key in ("input_text", "scored_candidates", "verification")
    ) and all(
        key in verification
        for key in ("accepted", "rejected", "abstained", "candidate_results")
    )
