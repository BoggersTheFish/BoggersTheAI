...
from ts_reasoner.calibration.calibrator import (
    ...
    for case in cases:
        ...
        for channel in CHANNELS:
            ...
            expected_activation = bool(
                expected_channels.get(channel, bool(observed.get("activated")))
            )
            trace_activation = bool(observed.get("activated"))
            label_activation = trace_activation
            label_resolution = (
                observed.get("resolution") if trace_activation else "not_activated"
            )
            rows.append(
                {
                    ...
                    "answer_ok": case["expected_answer_contains"].lower()
                    in output.final_answer.lower(),
                    ...
                    "hand_coded_resolution": observed.get(
                        "resolution", "not_activated"
                    ),
                    "trace_schema_valid": "tension_channels" in output.trace
                    and "typed_runtime" in output.trace,
                }
            )
    return rows
...
def evaluate_ablations(
    rows: list[dict[str, Any]], calibrator: TypedChannelCalibrator
) -> dict[str, Any]:
    ablations = {
        "hand_coded_baseline": _evaluate_rows(
            rows, calibrator, mode="hand_coded_baseline"
        ),
        "learned_activation": _evaluate_rows(
            rows, calibrator, mode="learned_activation"
        ),
        "learned_channel_weight": _evaluate_rows(
            rows, calibrator, mode="learned_channel_weight"
        ),
        "learned_resolver_priority": _evaluate_rows(
            rows, calibrator, mode="learned_resolver_priority"
        ),
...
def _evaluate_rows(
    rows: list[dict[str, Any]], calibrator: TypedChannelCalibrator, mode: str
) -> dict[str, Any]:
    predictions = [_predict_row(row, calibrator, mode) for row in rows]
    activation_accuracy = sum(p["activation_ok"] for p in predictions) / max(
        1, len(predictions)
    )
    resolver_accuracy = sum(p["resolver_ok"] for p in predictions) / max(
        1, len(predictions)
    )
    case_rows = _group_by_task(rows)
    answer_accuracy = sum(
        any(row["answer_ok"] for row in task_rows) for task_rows in case_rows.values()
    ) / max(1, len(case_rows))
    return {
        ...
        "reverse_fallacy_count": _failure_count(
            predictions, "reverse_invalid", "directionality"
        ),
        "identity_collapse_count": _failure_count(
            predictions, "identity_invalid", "identity_preservation"
        ),
        "unsupported_leap_count": _failure_count(
            predictions, "some_all_unsupported", "quantifier_scope"
        ),
        "abstention_correctness": round(_abstention_correctness(predictions), 4),
        "trace_schema_validity": round(
            sum(bool(row["trace_schema_valid"]) for row in rows) / max(1, len(rows)), 4
        ),
        "mean_predicted_channel_weight": round(
            sum(float(p["predicted_weight"]) for p in predictions)
...
def _predict_row(
    row: dict[str, Any], calibrator: TypedChannelCalibrator, mode: str
) -> dict[str, Any]:
    ...
    if mode == "hand_coded_baseline":
        ...
        resolution = (
            str(row["hand_coded_resolution"]) if activation else "not_activated"
        )
        weight = 1.0 if activation else 0.0
        ...
        weight = (
            1.0 / (1.0 + calibrator.priority_rank(row["channel"]))
            if activation
            else 0.0
        )
...
def _failure_count(
    predictions: list[dict[str, Any]], task_type: str, channel: str
) -> int:
    by_task = {
    ...
    return sum(
        1 for prediction in by_task.values() if not prediction["predicted_activation"]
    )
...
def _abstention_correctness(predictions: list[dict[str, Any]]) -> float:
    relevant = [
        ...
        and prediction["task_type"]
        in {"low_confidence_abstention", "some_all_unsupported"}
    ]
    if not relevant:
        return 1.0
    return sum(prediction["predicted_activation"] for prediction in relevant) / len(
        relevant
    )
