...
def extract_case_features(
    case: dict[str, Any], output: Any, channel: str
) -> dict[str, Any]:
    premises = [str(item) for item in case.get("premises", [])]
    relations = _extract_structural_relations(premises)
    query_subject, query_predicate, query_quantifier = _extract_structural_query(
        str(case.get("question", ""))
    )
    all_edges = [
        relation
        for relation in relations
        if relation["relation"] in {"all", "subset_of"}
    ]
    some_edges = [
        relation
        for relation in relations
        if relation["relation"] in {"some", "overlaps"}
    ]
    no_edges = [
        relation
        for relation in relations
        if relation["relation"] in {"no", "contradicts"}
    ]
    has_chain = _has_all_all_chain(all_edges)
    direct = _has_direct_relation(
        relations, query_subject, query_predicate, query_quantifier
    )
    transitive = _has_transitive_relation(all_edges, query_subject, query_predicate)
    reverse = (
        _has_transitive_relation(all_edges, query_predicate, query_subject)
        and not direct
    )
    some_all = _has_some_all_upgrade(
        some_edges, all_edges, query_subject, query_predicate, query_quantifier
    )
    ...
    features = {
        ...
        "channel_observed_initial_tension": float(
            channel_trace.get("initial_tension", 0.0)
        ),
        "channel_observed_final_tension": float(
            channel_trace.get("final_tension", 0.0)
        ),
...
def _has_direct_relation(
    relations, subject: str, predicate: str, quantifier: str
) -> bool:
...
def _has_some_all_upgrade(
    some_edges, all_edges, subject: str, predicate: str, quantifier: str
) -> bool:
    if quantifier != "all":
        return False
    return any(
...
def _has_direct_contradiction(relations) -> bool:
    for left in relations:
        for right in relations:
            ...
            if (
                same_pair
                and {left["relation"], right["relation"]} & {"no", "contradicts"}
                and {left["relation"], right["relation"]}
                & {"all", "some", "subset_of", "overlaps"}
            ):
                return True
    return False
...
def _extract_structural_relations(premises: list[str]) -> list[dict[str, str]]:
    ...
    for premise in premises:
        ...
        patterns = [
            (
                r"^every\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+is\s+(?:a|an)?\s*(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "all",
            ),
            (
                r"^all\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+are\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "all",
            ),
            (
                r"^(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+subset_of\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "subset_of",
            ),
            (
                r"^(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+equals\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "equals",
            ),
            (
                r"^(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+contradicts\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "contradicts",
            ),
            (
                r"^(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+overlaps\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)$",
                "overlaps",
            ),
        ]
...
def _extract_structural_query(question: str) -> tuple[str, str, str]:
    ...
    if identity_match:
        return (
            _norm_symbol(identity_match.group("s")),
            _norm_symbol(identity_match.group("o")),
            "identity",
        )
    patterns = [
        (
            r"\bis\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+subset_of\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)",
            "subset_of",
        ),
        (
            r"\bdoes\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+contradict\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)",
            "contradicts",
        ),
        (
            r"\bare\s+some\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+(?P<o>[A-Za-z][A-Za-z0-9_-]*)",
            "some",
        ),
        (
            r"\bis\s+each\s+(?P<s>[A-Za-z][A-Za-z0-9_-]*)\s+(?:a|an)?\s*(?P<o>[A-Za-z][A-Za-z0-9_-]*)",
            "all",
        ),
    ]
    for pattern, relation in patterns:
        ...
        if match:
            return (
                _norm_symbol(match.group("s")),
                _norm_symbol(match.group("o")),
                relation,
            )
    query = infer_query_relation(question)
    if query is not None:
        return (
            _norm_symbol(query.subject),
            _norm_symbol(query.predicate),
            query.quantifier,
        )
    return "", "", ""
...
def _structural_features(
    relations: list[dict[str, str]], subject: str, obj: str, query_relation: str
) -> dict[str, Any]:
    ...
    direct = any(
        edge["subject"] == subject and edge["object"] == obj for edge in relations
    )
    reverse_direct = any(
        edge["subject"] == obj and edge["object"] == subject for edge in relations
    )
    ...
    contradiction_any = any(
        edge["relation"] in {"no", "contradicts"} for edge in relations
    )
    quantifier_upgrade = query_relation == "all" and any(
        edge["relation"] in {"some", "overlaps"} for edge in path_edges + relevant_edges
    )
    identity_evidence = any(
        edge["relation"] == "equals"
        and {edge["subject"], edge["object"]} == {subject, obj}
        for edge in relations
    )
    return {
        ...
        "path_contains_some": float(
            "some" in path_relations or "overlaps" in path_relations
        ),
        "path_contains_no": float(
            "no" in path_relations or "contradicts" in path_relations
        ),
        "path_all_subset_chain_valid": float(
            bool(path_edges) and path_relations <= {"all", "subset_of", "equals"}
        ),
        "contradiction_on_query_path": float(contradiction_on_path),
        "contradiction_off_query_path": float(
            contradiction_any and not contradiction_on_path
        ),
        "contradiction_blocks_answer": float(
            contradiction_on_path and query_relation in {"all", "some", "subset_of"}
        ),
        "identity_evidence_exists": float(identity_evidence),
        "identity_claim_without_evidence": float(
            query_relation == "identity" and not identity_evidence
        ),
        "candidate_requires_transitive_closure": float(
            inferred and query_relation in {"all", "subset_of"}
        ),
        "candidate_requires_reverse_inference": float(
            bool(reverse_path) and not bool(path_edges)
        ),
        "candidate_requires_identity_collapse": float(
            query_relation == "identity" and bool(path_edges) and not identity_evidence
        ),
...
def _shortest_path(
    relations: list[dict[str, str]], source: str, target: str
) -> list[dict[str, str]]:
...
def _path_count(
    relations: list[dict[str, str]], source: str, target: str, max_depth: int = 5
) -> int:
...
def _contradiction_on_path(
    relations: list[dict[str, str]], query_nodes: set[str]
) -> bool:
    for edge in relations:
