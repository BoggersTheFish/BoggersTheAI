...
class KernelState:
    ...
    def from_dict(cls, payload: dict[str, Any] | None) -> "KernelState":
        payload = payload or {}
        return cls(
            accepted_common_ground=tuple(
                normalize_claims(
                    payload.get(
                        "accepted_common_ground", payload.get("accepted_claims", [])
                    )
                )
            ),
            quarantined_claims=tuple(
                normalize_claims(payload.get("quarantined_claims", []))
            ),
...
class EpistemicMicrokernel:
    def __init__(self, state: KernelState | dict[str, Any] | None = None) -> None:
        self._state = (
            state if isinstance(state, KernelState) else KernelState.from_dict(state)
        )
    ...
    def handle_request(self, request: KernelRequest) -> KernelDecision:
        ...
        if request.requested_action == "accept_claim":
            ...
            if not claim:
                ...
                state = replace(
                    self._state,
                    accepted_common_ground=(*self._state.accepted_common_ground, claim),
                )
                ...
                state = replace(
                    self._state, repair_targets=(*self._state.repair_targets, target)
                )
            ...
            state = replace(
                self._state, quarantined_claims=(*self._state.quarantined_claims, claim)
            )
            ...
            world_id = str(
                ...
                or f"world_{canonical_hash({'claim': claim})[:12]}"
            )
            branch = {
                "world_id": world_id,
            ...
            state = replace(
                self._state, branch_worlds=(*self._state.branch_worlds, branch)
            )
        ...
        state = replace(
            state,
            ledger_head=canonical_hash(
                {"previous": previous, "receipt": receipt, "state": state.to_dict()}
            ),
        )
        self._state = state
...
def run_userspace_app(
    ...
    *,
    ...
    if output.get("schema") != TS_OS_USERSPACE_APP_SCHEMA or not isinstance(
        candidates, list
    ):
        malformed = True
        candidates = []
    ...
    supported = sum(
        ...
        if decision.verifier_gate.get("passed") and decision.action == "accepted"
    )
    repairs = sum(
        len(decision.state_delta.get("repair_targets_added", []))
        for decision in decisions
    )
...
def run_continuum_scenario(scenario: dict[str, Any]) -> ContinuumReport:
    ...
    for idx, event in enumerate(scenario.get("events", [])):
        ...
        broken_edges = [
            (a, b)
            for a, b in edges
            if a in affected and b in affected and event_type == "shock"
        ]
        repairs = tuple(
            {"target": node, "repair_type": "restore_dependency"}
            for node in affected
            if event_type in {"shock", "missing_support"}
        )
        tension = (
            len(broken_edges)
            + len(invalid_nodes)
            + (2 if event_type == "contradiction" else 0)
        )
    ...
    return ContinuumReport(
        ...
        collapsed_worlds=[
            item.world_id for item in outcomes if item.outcome == "collapsed"
        ],
        quarantined_worlds=[
            item.world_id for item in outcomes if item.outcome == "quarantined"
        ],
        merge_candidates=[
            item.world_id for item in outcomes if item.outcome == "merge_candidate"
        ],
...
class ChannelManifest:
    ...
    def from_dict(cls, payload: dict[str, Any] | None) -> "ChannelManifest":
        payload = payload or {}
        return cls(
            verifier_contract_ids=tuple(
                str(item)
                for item in payload.get(
                    "verifier_contract_ids", ["typed_verifier_support"]
                )
            ),
            channel_contracts=tuple(
                str(item)
                for item in payload.get(
                    "channel_contracts", ["identity_preservation", "proof_boundary"]
                )
            ),
        )
...
class ProofGridNode:
    def __init__(
        ...
        self.manifest = (
            ...
            else ChannelManifest.from_dict(manifest)
        )
    ...
    def import_pack(self, pack: dict[str, Any]) -> PackImportDecision:
        ...
        if (
            pack.get("pack_hash") != expected
            or pack.get("schema") != KNOWLEDGE_PACK_SCHEMA
        ):
        ...
        compatible = set(incoming_manifest.verifier_contract_ids).issubset(
            set(self.manifest.verifier_contract_ids)
        )
        ...
        action = (
            ...
            else "quarantined" if quarantined else "checked"
        )
    ...
    def _import_receipt(
        ...
        receipt = {
            ...
            "hash_integrity_checked": action != "rejected"
            or reason == "bad_hash_or_schema",
        }
        receipt["receipt_hash"] = canonical_hash({**receipt, "receipt_hash": ""})
...
def run_alpha_scenario(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    if app_cmd:
        ...
        state = (
            userspace.decisions[-1].state
            if userspace.decisions
            else KernelState.from_dict(session.get("initial_state", {}))
        )
    else:
        kernel = EpistemicMicrokernel(session.get("initial_state", {}))
        for candidate in session.get("candidates", []):
            kernel.handle_request(
                KernelRequest(
                    requested_action=str(
                        candidate.get("requested_action", "accept_claim")
                    ),
                    candidate_payload=dict(candidate.get("payload", candidate)),
                    userspace_app_id="fixture",
                )
            )
        userspace = UserspaceRunResult(
            "userspace_completed",
            {"schema": TS_OS_USERSPACE_APP_SCHEMA},
            (),
            int(session.get("base_budget", 1)),
            {},
        )
        state = kernel.state
    ...
    receipt = {
        ...
        "all_gates_passed": continuum.contamination_count == 0
        and import_decision.receipt["candidate_graph_contamination_count"] == 0,
    }
    receipt["receipt_hash"] = canonical_hash({**receipt, "receipt_hash": ""})
    return {
        "release": "v10.5",
