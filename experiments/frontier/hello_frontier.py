...
def stable_hash(p): return hashlib.sha256(json.dumps(p, sort_keys=True).encode()).hexdigest()[:16]

# --- Use the enhanced rules with graph native (we edited core) ---
try:
    from core.graph.rules_engine import spawn_emergence, _graph_native_evolve, EMERGENCE_MAX_SPAWN
...
class SimpleGraph:
    ...
    def add_node(self, content, topics=None):
        ...
        self.nodes[nid] = type('N',(), {'id':nid, 'content':content, 'activation':0.25, 'base_strength':0.15, 'stability':0.6, 'topics':set(topics or []), 'collapsed':False})()
        return nid
    def add_edge(self, s, t, rel="is_a", w=0.9):
        self.edges.append(type('E',(), {'source_id':s, 'target_id':t, 'relation':rel, 'weight':w})())

def main():
    print("=== HELLO FRONTIER — Phase 0 Milestone ===")
    print("Goal: advanced deterministic TS reasoning + waves (graph native, no LLM core)\n")
    ...
    if HAS_REAL:
        ...
        # simple simulation
        for s in range(4):
            wave_trace.append({"step":s, "max_tension": round(0.05 + s*0.01, 3), "emergent": 1 if s==2 else 0})
    ...
    print("\n=== MILESTONE ACHIEVED ===")
    print("Phase 0 core: TS reasoning (proof + verifier) + wave physics (graph-native emergence) + receipt.")
