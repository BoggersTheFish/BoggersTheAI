...
class TSNode:
    def __init__(
        self, nid, content, topics=None, activation=0.2, base=0.15, stability=0.6
    ):
        self.id = nid
...
class TSGraph:
    ...
    def detect_tensions(self):
        return {
            nid: abs(n.activation - n.base_strength)
    ...
    def run_adaptive_waves(self, max_steps=8, tension_target=0.1):
        ...
        for s in range(max_steps):
            ...
            trace.append(
                {"step": s, "max_tension": round(max_t, 4), "emerged": len(emerged)}
            )
...
def verify_claim(premises, claim):
    parsed = [
        UnivRel("all", p.split(" are ")[0][4:], p.split(" are ")[1])
        for p in premises
        if p.lower().startswith("all ") and " are " in p
    ]
    path = universal_bridge_path(
        parsed, claim.get("subject", ""), claim.get("predicate", "")
    )
    passed = len(path) > 0
    return {
        "claim": claim,
...
class TensionProposerStub:
    def propose(self, high_tension_nodes):
        return [
            {"content": f"Intuition proposal for {n['content']}", "type": "candidate"}
            for n in high_tension_nodes[:2]
        ]
...
def execute_bogvm_plan(plan_desc, inputs):
    ...
    if "add" in plan_desc.lower() or "+" in plan_desc:
        ...
        return {
            "output": res,
            "verified": True,
            "note": "BOGVM execution complete, result matches claim",
        }
...
def main():
    ...
    for p in premises:
        sid = g.add_node(
            p.split(" are ")[0].replace("all ", "").replace("no ", "").strip()
        )
    ...
    for t in wave_trace:
        print(
            f"   step {t['step']}: tension={t['max_tension']}, emerged={t.get('emerged',0)}"
        )
    snap = g.snapshot()
