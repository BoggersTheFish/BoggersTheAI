...
v1.1 is a preregistered successor to the retired v1 contract.

No frozen evaluation seeds were inspected before this successor was defined.

v1.1 reuses without modification the v1:

- MemoryAlias-POMDP environment;
- deterministic binary stream generator;
- common tabular learner;
- fixed representations;
- training horizon;
- primary and secondary metrics;
- development seeds;
- frozen evaluation seeds;
- development-only fixed-baseline selection.

The frozen v1 development-selected baseline remains:

    FIXED-H4

The sole substantive experimental-design correction is the candidate repair
transition rule documented below.

The reason for that correction is preserved in the v1 RETIREMENT.md record.
...
When the current representation produces a verified obstruction, the proposal
mechanism may nominate a bounded candidate set containing any strictly deeper
permitted representation.

The proposal mechanism may not authorize a candidate and may not use frozen
evaluation outcomes to rank candidates.

The independent verifier evaluates the current representation against the
proposed deeper candidates using bounded evidence.

At most one representation may be authorized per repair event.

If multiple candidates satisfy the frozen verifier rule, the smallest
sufficient representation depth must be selected.

If no candidate satisfies the verifier rule, the proposal is rejected with
zero canonical representation effect.
