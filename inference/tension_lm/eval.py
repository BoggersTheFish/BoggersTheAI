...
def get_args():
    ...
    p.add_argument(
        "--text_file", default=None, help="Evaluate on a custom .txt file instead"
    )
    p.add_argument("--split", default="test", choices=["train", "validation", "test"])
    p.add_argument(
        "--max_tokens", default=None, type=int, help="Cap eval tokens (None = all)"
    )
...
def main():
    ...
    t0 = time.time()
    ppl = evaluate_ppl(
        model, ids, seq_len, args.batch_size, stride, device, cfg.vocab_size
    )
    print(f"\n{'─'*40}")
    print(f"  Model   : {arch}")
    print(
        f"  Dataset : {args.dataset if not args.text_file else args.text_file} [{args.split}]"
    )
    print(f"  PPL     : {ppl:.2f}")
