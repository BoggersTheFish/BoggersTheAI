...
def _check(runtime, tensor, name, dtype=np.float32):
    ...
    if tensor.dtype != np.dtype(dtype):
        raise ValueError(f"{name} must use {np.dtype(dtype)}")
...
def _run(runtime, name, count, args):
    ...
    kernel = runtime.kernel(WORKSPACE_BACKWARD_SOURCE, name)
    runtime.run_kernel(
        kernel,
        global_size=(runtime.round_up(count, local),),
        local_size=(local,),
        arguments=args,
    )
...
def fill_device(runtime, shape, value=0.0, *, output=None):
    output = _out(runtime, shape, output)
    _run(
        runtime,
        "fill_fp32",
        output.size,
        (output.buffer, np.float32(value), np.uint32(output.size)),
    )
    return output


def add_device(runtime, left, right, *, output=None):
    _check(runtime, left, "left")
    _check(runtime, right, "right")
    if left.shape != right.shape:
        raise ValueError("add shapes must match")
    output = _out(runtime, left.shape, output)
    _run(
        runtime,
        "add_fp32",
        left.size,
        (left.buffer, right.buffer, output.buffer, np.uint32(left.size)),
    )
    return output


def scale_inplace_device(runtime, tensor, scale):
    _check(runtime, tensor, "tensor")
    _run(
        runtime,
        "scale_inplace_fp32",
        tensor.size,
        (tensor.buffer, np.float32(scale), np.uint32(tensor.size)),
    )


def concatenate_backward_device(runtime, grad, left_columns):
    _check(runtime, grad, "grad")
    rows, cols = grad.shape
    rc = cols - left_columns
    gl = _out(runtime, (rows, left_columns))
    gr = _out(runtime, (rows, rc))
    _run(
        runtime,
        "concat_backward_fp32",
        grad.size,
        (
            grad.buffer,
            gl.buffer,
...
def broadcast_rows_backward_device(runtime, grad, batches, repeats):
    _check(runtime, grad, "grad")
    f = grad.shape[1]
    o = _out(runtime, (batches, f))
    _run(
        runtime,
        "broadcast_rows_backward_fp32",
        o.size,
        (grad.buffer, o.buffer, np.uint32(batches), np.uint32(repeats), np.uint32(f)),
    )
    return o
...
def table_row_backward_device(runtime, grad, table_shape, row):
    _check(runtime, grad, "grad")
    o = _out(runtime, table_shape)
    _run(
        runtime,
        "table_row_backward_fp32",
        o.size,
        (
            grad.buffer,
            o.buffer,
            np.uint32(grad.shape[0]),
            np.uint32(table_shape[0]),
            np.uint32(table_shape[1]),
            np.uint32(row),
        ),
    )
    return o
...
def gelu_backward_device(runtime, inputs, grad):
    o = _out(runtime, inputs.shape)
    _run(
        runtime,
        "gelu_backward_fp32",
        o.size,
        (inputs.buffer, grad.buffer, o.buffer, np.uint32(o.size)),
    )
    return o


def layer_norm_backward_device(runtime, inputs, weight, grad, epsilon=1e-5):
    rows = inputs.size // inputs.shape[-1]
    c = inputs.shape[-1]
    gx = _out(runtime, inputs.shape)
    gw = _out(runtime, (c,))
    gb = _out(runtime, (c,))
    _run(
        runtime,
        "layer_norm_backward_input_fp32",
        rows,
        (
            inputs.buffer,
            weight.buffer,
    ...
    _run(
        runtime,
        "layer_norm_backward_param_fp32",
        c,
        (
            inputs.buffer,
            grad.buffer,
...
def softmax_backward_device(runtime, output, grad):
    rows = output.size // output.shape[-1]
    c = output.shape[-1]
    o = _out(runtime, output.shape)
    _run(
        runtime,
        "softmax_backward_fp32",
        rows,
        (output.buffer, grad.buffer, o.buffer, np.uint32(rows), np.uint32(c)),
    )
    return o


def workspace_reduce_backward_device(runtime, workspace, grad_mean, grad_max):
    b, n, f = workspace.shape
    o = _out(runtime, workspace.shape)
    _run(
        runtime,
        "reduce_backward_fp32",
        o.size,
        (
            workspace.buffer,
            grad_mean.buffer,
...
def gather_backward_device(runtime, grad, indices, input_shape, shared=False):
    ...
    n = input_shape[-2]
    f = input_shape[-1]
    o = _out(runtime, input_shape)
    _run(
        runtime,
        "gather_backward_fp32",
        o.size,
        (
            grad.buffer,
            indices.buffer,
...
def scatter_backward_device(runtime, grad, indices):
    b, n, f = grad.shape
    k = indices.shape[1]
    gi = _out(runtime, grad.shape)
    gu = _out(runtime, (b, k, f))
    _run(
        runtime,
        "scatter_backward_input_fp32",
        gi.size,
        (
            grad.buffer,
            indices.buffer,
...
def row_delta_norm_backward_device(runtime, left, right, output, grad):
    rows, c = left.shape
    gl = _out(runtime, left.shape)
    gr = _out(runtime, right.shape)
    _run(
        runtime,
        "row_norm_backward_fp32",
        left.size,
        (
            left.buffer,
            right.buffer,
...
def tension_update_broadcast_backward_device(runtime, state, proposal, tension, grad):
    rows, c = state.shape
    gs = _out(runtime, state.shape)
    gp = _out(runtime, state.shape)
    gt = _out(runtime, tension.shape)
    _run(
        runtime,
        "tension_backward_fp32",
        state.size,
        (
            state.buffer,
            proposal.buffer,
...
def batched_dot_backward_device(runtime, query, slots, grad, scale=1.0):
    b, n, f = slots.shape
    gq = _out(runtime, query.shape)
    gv = _out(runtime, slots.shape)
    _run(
        runtime,
        "batched_dot_backward_fp32",
        slots.size,
        (
            query.buffer,
            slots.buffer,
...
def tension_penalty_backward_device(runtime, tension, grad):
    gs = _out(runtime, grad.shape)
    gt = _out(runtime, tension.shape)
    _run(
        runtime,
        "penalty_backward_fp32",
        grad.size,
        (tension.buffer, grad.buffer, gs.buffer, gt.buffer, np.uint32(grad.size)),
    )
    return gs, gt
...
def weighted_sum_backward_device(runtime, weights, values, grad):
    b, n, f = values.shape
    gw = _out(runtime, weights.shape)
    gv = _out(runtime, values.shape)
    _run(
        runtime,
        "weighted_backward_fp32",
        values.size,
        (
            weights.buffer,
            values.buffer,
...
def embedding_forward_device(runtime, ids, weight, *, output=None):
    _check(runtime, ids, "ids", np.int32)
    count = ids.size
    f = weight.shape[1]
    output = _out(runtime, ids.shape + (f,), output)
    _run(
        runtime,
        "embedding_forward_fp32",
        output.size,
        (ids.buffer, weight.buffer, output.buffer, np.uint32(count), np.uint32(f)),
    )
    return output


def embedding_backward_device(runtime, ids, grad, weight_shape):
    o = _out(runtime, weight_shape)
    _run(
        runtime,
        "embedding_backward_fp32",
        o.size,
        (
            ids.buffer,
            grad.buffer,
            o.buffer,
            np.uint32(ids.size),
            np.uint32(weight_shape[0]),
            np.uint32(weight_shape[1]),
        ),
    )
    return o
...
def cross_entropy_device(runtime, logits, targets):
    _check(runtime, targets, "targets", np.int32)
    rows, c = logits.shape
    losses = _out(runtime, (rows,))
    grad = _out(runtime, logits.shape)
    _run(
        runtime,
        "cross_entropy_fp32",
        rows,
...
def route_scores_backward_device(runtime, query, keys, grad, scale):
    ...
    n = keys.shape[0]
    gq = _out(runtime, query.shape)
    gk = _out(runtime, keys.shape)
    _run(
        runtime,
        "route_backward_q_fp32",
        gq.size,
        (
            keys.buffer,
            grad.buffer,
...
def clip_gradients_device(runtime, gradients, max_norm=1.0):
    ...
    for gradient in gradients:
        _check(runtime, gradient, "gradient")
    parts = _out(runtime, (len(gradients),))
    norm = _out(runtime, (1,))
    for index, gradient in enumerate(gradients):
        _run(
            ...
            "sum_squares_at_fp32",
            1,
            (gradient.buffer, parts.buffer, np.uint32(index), np.uint32(gradient.size)),
        )
    _run(
        runtime,
        "finish_global_norm_fp32",
        1,
        (parts.buffer, norm.buffer, np.uint32(len(gradients))),
    )
    for gradient in gradients:
        _run(
            runtime,
            "clip_from_parts_fp32",
            gradient.size,
            (
                gradient.buffer,
                parts.buffer,
                np.uint32(len(gradients)),
                np.float32(max_norm),
                np.uint32(gradient.size),
            ),
        )
    return norm


def training_auxiliary_loss_device(
    runtime,
    pre_tensions,
    selected_indices,
    num_slots,
    tension_weight=0.001,
    usage_weight=0.001,
):
    pre_tensions = list(pre_tensions)
    selected_indices = list(selected_indices)
    if not pre_tensions or not selected_indices:
        raise ValueError("diagnostic tensors are required")
    total = fill_device(runtime, (1,))
    counts = fill_device(runtime, (num_slots,))
    loss = fill_device(runtime, (1,))
    tension_count = 0
    index_count = 0
    for tensor in pre_tensions:
        _check(runtime, tensor, "pre_tension")
        tension_count += tensor.size
        _run(
            runtime,
            "accumulate_sum_fp32",
            1,
            (tensor.buffer, total.buffer, np.uint32(tensor.size)),
        )
    for tensor in selected_indices:
        _check(runtime, tensor, "selected_indices", np.int32)
        index_count += tensor.size
        _run(
            runtime,
            "accumulate_index_counts_fp32",
            num_slots,
            (
                tensor.buffer,
                counts.buffer,
                np.uint32(tensor.size),
                np.uint32(num_slots),
            ),
        )
    _run(
        ...
        (
            ...
            counts.buffer,
            loss.buffer,
            np.uint32(tension_count),
            np.uint32(index_count),
            np.uint32(num_slots),
            np.float32(tension_weight),
            np.float32(usage_weight),
        ),
    )
    return loss
