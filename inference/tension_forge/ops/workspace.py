...
def _check(
    runtime: TensionForgeRuntime, tensor: DeviceTensor, name: str, *, dtype=np.float32
) -> None:
...
def layer_norm_device(runtime, inputs, weight, bias, *, epsilon=1e-5, output=None):
    ...
    if (
        inputs.ndim < 2
        or weight.shape != (inputs.shape[-1],)
...
def softmax_device(runtime, inputs, *, output=None):
    ...
    _run(
        ...
        "softmax_rows_fp32",
        rows,
        (inputs.buffer, output.buffer, np.uint32(rows), np.uint32(columns)),
    )
    return output
...
def gather_slots_device(runtime, inputs, indices, *, shared=False, output=None):
    ...
    if shared:
        ...
        if inputs.ndim != 3 or inputs.shape[0] != batches:
            raise ValueError(
                "batched gather input must have shape [batch, slots, features]"
            )
...
def broadcast_table_row_device(runtime, table, row, output_rows, *, output=None):
    ...
    _run(
        runtime,
        "broadcast_table_row_fp32",
        output.size,
        (
            table.buffer,
            output.buffer,
            np.uint32(output_rows),
            np.uint32(features),
            np.uint32(row),
        ),
    )
...
def gelu_device(runtime, inputs, *, output=None):
    ...
    _run(
        ...
        "gelu_fp32",
        inputs.size,
        (inputs.buffer, output.buffer, np.uint32(inputs.size)),
    )
    return output
...
def scale_device(runtime, inputs, scale, *, output=None):
    ...
    _run(
        ...
        "scale_fp32",
        inputs.size,
        (inputs.buffer, output.buffer, np.float32(scale), np.uint32(inputs.size)),
    )
    return output
...
def row_delta_norm_device(runtime, left, right, *, output=None):
    ...
    output = _output(runtime, (rows, 1), output)
    _run(
        runtime,
        "row_delta_norm_fp32",
        rows,
        (left.buffer, right.buffer, output.buffer, np.uint32(rows), np.uint32(columns)),
    )
    return output


def tension_update_broadcast_device(runtime, state, proposal, gate, *, output=None):
    _check(runtime, state, "state")
    _check(runtime, proposal, "proposal")
    _check(runtime, gate, "gate")
    if (
        state.ndim != 2
        or proposal.shape != state.shape
        or gate.shape != (state.shape[0], 1)
    ):
        raise ValueError("broadcast tension update shapes are incompatible")
    rows, columns = state.shape
    output = _output(runtime, state.shape, output)
    _run(
        runtime,
        "tension_update_broadcast_fp32",
        state.size,
        (
            state.buffer,
            proposal.buffer,
...
def batched_dot_device(runtime, query, slots, *, scale=1.0, output=None):
    _check(runtime, query, "query")
    _check(runtime, slots, "slots")
    if (
        query.ndim != 2
        or slots.ndim != 3
        or query.shape[0] != slots.shape[0]
        or query.shape[1] != slots.shape[2]
    ):
        raise ValueError("batched dot shapes are incompatible")
    batches, count, features = slots.shape
    output = _output(runtime, (batches, count), output)
    _run(
        runtime,
        "batched_dot_fp32",
        output.size,
...
def add_tension_penalty_device(runtime, scores, tension, *, output=None):
    _check(runtime, scores, "scores")
    _check(runtime, tension, "tension")
