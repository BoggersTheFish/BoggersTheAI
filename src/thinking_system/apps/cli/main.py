...
# Repo root: src/thinking_system/apps/cli/main.py -> parents[4]
...
def run_demo(args: argparse.Namespace) -> int:
    ...
    print("========================================================================")
    print(
        "Executing verifier-gated transaction pipeline under residual accounting...\n"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    demo_parser = subparsers.add_parser(
        "demo", help="Run canonical offline verifier-gated demo"
    )
    demo_parser.add_argument(
        "--json", action="store_true", help="Output raw JSON receipts"
    )

    # Command: replay
    replay_parser = subparsers.add_parser(
        "replay", help="Audit and replay a transaction receipt"
    )
    replay_parser.add_argument("receipt_file", help="Path to receipt JSON file")

    # Command: chat
    subparsers.add_parser("chat", help="Run interactive chat interface")

    # Command: version
    subparsers.add_parser("version", help="Show version and authority boundary")
    ...
    if args.command == "demo" or args.command is None:
        ...
        print(
            "Thinking System v0.5.0-alpha.1 (slug: thinking-system, namespace: thinking_system)"
        )
        print(
            "Authority Boundary: Generated language is NOT proof. State transitions are verifier-gated."
        )
        return 0
    else:
        parser.print_help()
