...
from BoggersTheAI.interface.runtime import BoggersRuntime
...
def run_flat_rag(runtime: BoggersRuntime, query: str, k: int = 5) -> dict:
    ...
    if not query_emb:
        # Fallback to lexical
        context_nodes = [
            node for node in runtime.graph.nodes.values() if not node.collapsed
        ][:k]
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate TS Engine vs. flat RAG baseline."
    )
    parser.add_argument(
        "--output", type=str, default="benchmark_report.md", help="Output file path"
    )
