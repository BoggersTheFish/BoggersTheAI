...
from .bogos import Workspace
from .genesis import ZERO_HASH, Genesis
...
from .store import _directory_hash, read_store_index
...
class HyperGenesis:
    ...
    def ledger_verify(self) -> dict:
        ...
        for state_path in sorted(self.genesis.states_dir.glob("*.json")):
            try:
                ...
                if (
                    _stable_hash({"files": state["files"]}) != state["root_sha256"]
                    or state_path.stem != state["root_sha256"]
                ):
                    failures.append(
                        {
                            "path": str(state_path),
                            "reason": "state manifest root mismatch",
                        }
                    )
                for entry in state["files"].values():
                    ...
                    if (
                        ...
                        or _file_hash(object_path) != entry["object_sha256"]
                    ):
                        failures.append(
                            {
                                "path": str(object_path),
                                "reason": "state object hash mismatch",
                            }
                        )
    ...
    def prove_file(self, path: str, root: str | None = None) -> dict:
        ...
        failures = (
            []
            if entry and (self.genesis.objects_dir / entry["object_sha256"]).is_file()
            else [{"path": path, "reason": "file not proven by state root"}]
        )
        return {
            ...
            "entry": entry,
            "object_verified": not failures
            and hashlib.sha256(
                (self.genesis.objects_dir / entry["object_sha256"]).read_bytes()
            ).hexdigest()
            == entry["object_sha256"],
            "failures": failures,
            "execution_status": "completed" if not failures else "blocked",
        ...
        for action in plan:
            try:
                if action["action"] == "write":
                    result = self.genesis.fs_write(
                        action["path"], action["data"], capability="BogPilot"
                    )
                    ...
                    result = {
                        ...
                        "failures": [
                            {"path": action["action"], "reason": "unknown pilot action"}
                        ],
                    }
            except Exception as exc:
                result = {
                    "execution_status": "blocked",
                    "failures": [{"path": str(action), "reason": str(exc)}],
                }
            results.append({"proposal": action, "result": result})
        accepted_or_blocked = all(
            item["result"].get("execution_status") in {"completed", "blocked"}
            for item in results
        )
        return self.genesis.record(
            "bog-pilot",
    ...
    def verify_proof(path: str | Path) -> dict:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                if manifest.get(
                    "format"
                ) != "BOGPROOF-bundle-10.0" or actual != manifest.get("files"):
                    failures.append(
                        {
                            "path": str(path),
                            "reason": "portable proof manifest mismatch",
                        }
                    )
                if manifest.get("final_receipt_sha256") != _file_hash(
                    root / "final_receipt.json"
                ):
                    failures.append(
                        {
                            "path": "final_receipt.json",
                            "reason": "portable final receipt hash mismatch",
                        }
                    )
                ...
                if not verify_object_signature(
                    registry_unsigned, registry_signature, trusted
                )["verified"]:
                    failures.append(
                        {
                            "path": "registry/index.json",
                            "reason": "portable registry signature invalid",
                        }
                    )
                for key, item in registry_unsigned.get("packages", {}).items():
                    ...
                    if not verify_object_signature(
                        receipt_unsigned, receipt_signature, trusted
                    )["verified"]:
                        failures.append(
                            {
                                "path": key,
                                "reason": "portable package signature invalid",
                            }
                        )
                    if (
                        _directory_hash(bundle) != item["bundle_sha256"]
                        or receipt["bundle_sha256"] != item["bundle_sha256"]
                    ):
                        failures.append(
                            {
                                "path": key,
                                "reason": "portable package bundle hash mismatch",
                            }
                        )
                    for dependency in receipt.get("dependencies", []):
                        if dependency not in registry_unsigned.get("packages", {}):
                            failures.append(
                                {
                                    "path": key,
                                    "reason": f"portable dependency missing: {dependency}",
                                }
                            )
                lock = json.loads((root / "bog.lock").read_text())
                lock_unsigned = dict(lock)
                lock_signature = lock_unsigned.pop("signature", {})
                if not verify_object_signature(lock_unsigned, lock_signature, trusted)[
                    "verified"
                ]:
                    failures.append(
                        {
                            "path": "bog.lock",
                            "reason": "portable lock signature invalid",
                        }
                    )
                if lock_unsigned.get("registry_sha256") != _stable_hash(
                    registry_unsigned
                ) or lock_unsigned.get("packages") != registry_unsigned.get("packages"):
                    failures.append(
                        {
                            "path": "bog.lock",
                            "reason": "portable lock does not pin registry",
                        }
                    )
                previous = ZERO_HASH
                entries = []
                for ledger_path in sorted((root / "ledger").glob("*.json")):
                    ...
                    if (
                        ...
                        or _stable_hash(payload) != receipt_hash
                    ):
                        failures.append(
                            {
                                "path": ledger_path.name,
                                "reason": "portable ledger chain mismatch",
                            }
                        )
                    if not verify_object_signature(unsigned, signature, trusted)[
                        "verified"
                    ]:
                        failures.append(
                            {
                                "path": ledger_path.name,
                                "reason": "portable ledger signature invalid",
                            }
                        )
                ...
                if not verify_object_signature(
                    unsigned_final, final_signature, trusted
                )["verified"]:
                    failures.append(
                        {
                            "path": "final_receipt.json",
                            "reason": "final receipt signature invalid",
                        }
                    )
                if final.get("genesis_session_root") != manifest.get(
                    "genesis_session_root"
                ):
                    failures.append(
                        {
                            "path": "final_receipt.json",
                            "reason": "portable session root linkage mismatch",
                        }
                    )
                if final.get("final_state_root_sha256") != manifest.get(
                    "final_state_root_sha256"
                ):
                    failures.append(
                        {
                            "path": "final_receipt.json",
                            "reason": "portable final state linkage mismatch",
                        }
                    )
                ...
                if virtual_root != manifest.get("final_state_root_sha256"):
                    failures.append(
                        {
                            "path": "states",
                            "reason": "portable replay final root mismatch",
                        }
                    )
                    ...
                    failures.append(
                        {
                            ...
                            "reason": "portable final state manifest mismatch",
                        }
                    )
                for entry in state["files"].values():
                        ...
                        failures.append(
                            {
                                ...
                                "reason": "portable state object mismatch",
                            }
                        )
            except (
                OSError,
        ...
        return {
            ...
            "final_state_root_sha256": (
                manifest.get("final_state_root_sha256")
    ...
    def demo(self) -> dict:
        ...
        lock = self.genesis.create_lock()
        installs = [
            self.genesis.install(key)
            for key in ("genesis-lib-1.0.0", "note-app-1.0.0", "built-cell-app-1.0.0")
        ]
        ...
        note = self.genesis.fs_write(
            "notes/today.txt", "HyperGenesis portable note\n", capability="demo"
        )
        ...
        provisional = {
            ...
            "all_packages_signed": all(
                self.genesis._install_signature_ok(item) for item in installs
            ),
            ...
            "bogcell_raw_syscall_surface_absent": cell_run["process_receipt"][
                "raw_syscall_surface"
            ]
            == [],
            "brokered_app_capabilities_verified": brokered["execution_status"]
            == "completed",
            "copy_on_write_state_verified": bool(diff["changed_paths"])
            and file_proof["execution_status"] == "completed"
            and ledger["state_history_verified"],
            "state_object_tamper_blocked": state_tamper["execution_status"]
            == "blocked",
            ...
            "ai_proposed_action_verified_or_blocked": pilot[
                "ai_proposed_action_verified_or_blocked"
            ],
            ...
            "proof": {
                "build": build_receipt,
                "cell": cell_run["receipt_hash"],
                "brokered": brokered_event["receipt_hash"],
            },
            "execution_status": "blocked",
        }
        ...
        checks = {
            **{
            ...
            "final_roots_match": verified["final_state_root_sha256"]
            == provisional["final_state_root_sha256"],
        }
        final = {
            "format": "BOGOS-HyperGenesis-final-receipt-10.0",
            **checks,
    ...
    def _plan(self, prompt: str) -> list[dict]:
        plan = [
            {
                "action": "write",
                "path": "pilot/approved.txt",
                "data": "BogPilot proposed this verified write",
            }
        ]
