...
class BogPilotSwarm:
    ...
    def tournament(
        self, hypothesis: str, candidates: list[dict], budget: dict | None = None
    ) -> dict:
        budget = {
            "cells": 32,
            "cost": 10_000,
        ...
        for index, proposal in enumerate(candidates[: budget["cells"]], 1):
            ...
            if action not in {"write", "reject"}:
                failures.append(
                    {
                        "path": str(action),
                        "reason": "candidate action is outside verifier surface",
                    }
                )
            if cost < 0 or total_cost > budget["cost"]:
                failures.append(
                    {
                        "path": str(index),
                        "reason": "candidate exceeds swarm cost budget",
                    }
                )
            if int(proposal.get("memory", 0)) > budget["memory"]:
                failures.append(
                    {
                        "path": str(index),
                        "reason": "candidate exceeds swarm memory budget",
                    }
                )
            if action == "write" and (
                not proposal.get("path") or ".." in proposal["path"].split("/")
            ):
                failures.append(
                    {
                        "path": str(proposal.get("path")),
                        "reason": "candidate write path is unsafe",
                    }
                )
        ...
        eligible = [
            ...
            and cell["proposal"]["action"] != "reject"
        ]
        eligible.sort(
            key=lambda cell: (-cell["score"], cell["cost"], cell["proof_sha256"])
        )
        selected = eligible[0] if eligible else None
        admission = None
        if selected:
            proposal = selected["proposal"]
            admission = self.genesis.fs_write(
                proposal["path"], proposal.get("data", ""), capability="BogPilotSwarm"
            )
        receipt = {
            ...
            "execution_status": (
                "completed"
                if selected and admission["execution_status"] == "completed"
                else "blocked"
            ),
        }
    ...
    def replay(receipt: dict) -> dict:
        ...
        for cell in cells:
            proof = {
                key: cell[key]
                for key in ("index", "proposal", "score", "cost", "failures")
            }
            if cell.get("proof_sha256") != _stable_hash(proof):
                failures.append(
                    {
                        "path": str(cell.get("index")),
                        "reason": "swarm cell proof mismatch",
                    }
                )
        eligible = [
            cell
            ...
            and cell.get("proposal", {}).get("action") != "reject"
        ]
        eligible.sort(
            key=lambda cell: (-cell["score"], cell["cost"], cell["proof_sha256"])
        )
        ...
        if expected != receipt.get("selected_proof_sha256"):
            failures.append(
                {"path": "selection", "reason": "swarm best-path selection mismatch"}
            )
        return {
