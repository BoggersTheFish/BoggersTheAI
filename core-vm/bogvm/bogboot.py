...
class BogBoot:
    ...
    def boot(self, manifest: dict | None = None) -> dict:
        ...
        state = {
            ...
            "quarantined_claims": [],
            "devices": {
                name: {"event_count": 0, "last_payload_sha256": None}
                for name in sorted(manifest.get("devices", {}))
            },
        }
        ...
        receipt = {
            ...
            "hardware_state_root_sha256": (
                state["state_root_sha256"] if not failures else None
            ),
    ...
    def irq_claim(
        ...
        if device is None:
            failures.append(
                {
                    "path": source,
                    "reason": "device is not declared by hardware manifest",
                }
            )
        if required not in capabilities:
            failures.append(
                {"path": source, "reason": f"missing hardware capability: {required}"}
            )
        if not isinstance(tick, int) or tick <= state["tick"]:
            failures.append(
                {
                    "path": str(tick),
                    "reason": "hardware tick must increase monotonically",
                }
            )
        ...
        if failures:
            ...
            state["devices"][source]["last_payload_sha256"] = claim_core[
                "raw_payload_sha256"
            ]
        state["state_root_sha256"] = self._state_root(state)
        _write_json(self.state_path, state)
        ...
        try:
                ...
                failures.append(
                    {
                        ...
                        "reason": "hardware manifest hash mismatch",
                    }
                )
                ...
                failures.append(
                    {
                        ...
                        "reason": "hardware state root mismatch",
                    }
                )
            claims = sorted(self.claims_dir.glob("*.json")) + sorted(
            ...
            if len(claims) != state.get("claim_sequence"):
                failures.append(
                    {
                        "path": str(self.dir),
                        "reason": "hardware claim sequence mismatch",
                    }
                )
            for path in claims:
                ...
                if claim.get("claim_id") != _stable_hash(
                    {
                        ...
                        for key in (
                            ...
                            "decoded_event",
                            "capability_required",
                            "capabilities_presented",
                            "state_root_before",
                        )
                    }
                ):
                    failures.append(
                        {"path": str(path), "reason": "IRQ claim hash mismatch"}
                    )
    ...
    def _load(self) -> tuple[dict, dict]:
        ...
        return json.loads(self.state_path.read_text()), json.loads(
    ...
    def _validate_manifest(manifest: dict) -> list[dict]:
        ...
        if manifest.get("platform") != "qemu-virt-reference":
            failures.append(
                {
                    "path": "platform",
                    "reason": "BogBoot v0.1 supports only qemu-virt-reference",
                }
            )
        required = {"serial", "timer"}
        if not required.issubset(manifest.get("devices", {})):
            failures.append(
                {"path": "devices", "reason": "serial and timer devices are required"}
            )
        ranges = []
        for entry in manifest.get("memory_map", []):
            ...
            if (
                ...
                failures.append(
                    {
                        "path": "memory_map",
                        "reason": "memory ranges require non-negative start and positive size",
                    }
                )
            else:
        ...
        for previous, current in zip(sorted(ranges), sorted(ranges)[1:]):
            if current[0] < previous[1]:
                failures.append(
                    {
                        "path": "memory_map",
                        "reason": f"overlapping memory ranges: {previous[2]} and {current[2]}",
                    }
                )
        return failures
    ...
    def _state_root(state: dict) -> str:
        return _stable_hash(
            {key: value for key, value in state.items() if key != "state_root_sha256"}
        )
