...
def public_key_info(public_key_path: str | Path) -> dict:
    public_bytes = base64.b64decode(
        Path(public_key_path).read_text().strip(), validate=True
    )
...
def sign_object(obj: dict, private_key_path: str | Path) -> dict:
    private_bytes = base64.b64decode(
        Path(private_key_path).read_text().strip(), validate=True
    )
...
def verify_object_signature(
    obj: dict, signature: dict, trusted_public_keys: list[str | Path]
) -> dict:
    if (
        signature.get("format") != SIGNATURE_FORMAT
    ...
    for path in trusted_public_keys:
        try:
            public_bytes = base64.b64decode(
                Path(path).read_text().strip(), validate=True
            )
    ...
    return {
        "verified": False,
        "key_id": signature.get("key_id"),
        "reason": "signature is not valid for a trusted key",
    }
