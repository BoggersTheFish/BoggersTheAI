...
def vertical_demo(workspace: Any) -> dict:
    ...
    keyboard = boot.irq_claim(
        "keyboard", "a", {"key": "a", "pressed": True}, ["hardware.keyboard.input"]
    )
    blocked_irq = boot.irq_claim("block_device", "sector:7", {"sector": 7}, [])
    swarm = BogPilotSwarm(workspace)
    tournament = swarm.tournament(
        "choose verified system action",
        [
            {
                "action": "write",
                "path": "vertical/action-a.txt",
                "data": "A",
                "score": 7,
                "cost": 2,
            },
    ...
    mesh.propose(
        ...
        context={"branch": "main"},
        support=2,
        capability_scope=["registry.publish"],
    )
    conflict = mesh.propose(
        "bogos/vertical",
        {"state": "C"},
        context={"branch": "main"},
        support=2,
    ...
    return boot.genesis.record(
        "verifier-first-vertical",
        {
