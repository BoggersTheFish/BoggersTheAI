...
class Genesis:
    ...
    def boot(self) -> dict:
        ...
        material = {
            ...
            "installed_package_index_sha256": self._file_hash(
                self.workspace.bogos / "store" / "index.json"
            ),
                    ...
                    "failures": [
                        {
                            ...
                            "reason": "registry source is missing index.json",
                        }
                    ],
    ...
    def verify_registry(self, required: bool = True) -> dict:
        ...
        try:
            ...
            failures = (
                []
                if result["verified"]
                else [{"path": str(path), "reason": result["reason"]}]
            )
            if signed.get("format") != REGISTRY_FORMAT:
                failures.append(
                    {"path": str(path), "reason": "unsupported Genesis registry format"}
                )
            for key, item in signed.get("packages", {}).items():
                ...
                if not bundle.is_relative_to(self.registry_dir.resolve()):
                    failures.append(
                        {
                            "path": key,
                            "reason": "registry package path escapes registry",
                        }
                    )
                    continue
                ...
                if receipt["bundle_sha256"] != item["bundle_sha256"]:
                    failures.append(
                        {"path": key, "reason": "registry package bundle hash mismatch"}
                    )
                if _directory_hash(bundle) != item["bundle_sha256"]:
                    failures.append(
                        {
                            "path": key,
                            "reason": "registry package content hash mismatch",
                        }
                    )
                if self._stable_hash(receipt) != item["receipt_hash"]:
                    failures.append(
                        {
                            "path": key,
                            "reason": "registry package receipt hash mismatch",
                        }
                    )
                unsigned_receipt = dict(receipt)
                package_signature = unsigned_receipt.pop("signature", {})
                if not verify_object_signature(
                    unsigned_receipt, package_signature, self.trusted_keys
                )["verified"]:
                    failures.append(
                        {
                            "path": key,
                            "reason": "registry package signature is not trusted",
                        }
                    )
                expected = {
                    "name": receipt["name"],
                ...
                if any(item.get(field) != value for field, value in expected.items()):
                    failures.append(
                        {
                            "path": key,
                            "reason": "registry package metadata does not match signed receipt",
                        }
                    )
            return {
    ...
    def create_lock(self) -> dict:
        ...
        lock = {
            "format": LOCK_FORMAT,
            "registry_sha256": self._stable_hash(
                {k: v for k, v in registry.items() if k != "signature"}
            ),
    ...
    def verify_lock(self, required: bool = True) -> dict:
        ...
        try:
            ...
            failures = (
                []
                if result["verified"]
                else [{"path": str(self.lock_path), "reason": result["reason"]}]
            )
            if signed["registry_sha256"] != registry.get("registry_sha256"):
                failures.append(
                    {
                        "path": str(self.lock_path),
                        "reason": "lockfile registry hash mismatch",
                    }
                )
            if signed["trust_key_ids"] != sorted(self._trusted_key_ids()):
                failures.append(
                    {
                        "path": str(self.lock_path),
                        "reason": "lockfile trust keys mismatch",
                    }
                )
            if signed["packages"] != self._registry_index().get("packages"):
                failures.append(
                    {
                        "path": str(self.lock_path),
                        "reason": "lockfile package pins do not match registry",
                    }
                )
            return {
    ...
    def install(self, package: str) -> dict:
        ...
        if entry is None:
            failures.append(
                {"path": package, "reason": "package is not pinned by bog.lock"}
            )
        install_receipt = None
        if not failures:
            install_receipt = self.workspace.install_package(
                self.registry_dir / entry["bundle"]
            )
        ...
        return self.record(
            "install",
            {
                ...
                "package": package,
                "registry_signature_verified": registry_check[
                ...
                "dependencies_verified": bool(
                    install_receipt
                    and install_receipt["execution_status"] == "completed"
                ),
                "failures": failures,
                "execution_status": "completed" if not failures else "blocked",
            },
    ...
    def fs_read(self, path: str, capability: str = "shell") -> tuple[bytes, dict]:
        ...
        failures = (
            []
            if entry
            else [{"path": rel, "reason": "file does not exist in Genesis state"}]
        )
        data = (
            (self.objects_dir / entry["object_sha256"]).read_bytes() if entry else b""
        )
        receipt = self.record(
            "fs-read",
            {
    ...
    def rollback(self, selector: str | int) -> dict:
        ...
        if not target_root or not (self.states_dir / f"{target_root}.json").exists():
            failures.append(
                {
                    "path": str(selector),
                    "reason": "receipt does not identify a recoverable state root",
                }
            )
        ...
        return self.record(
            "rollback",
            {
                ...
                "rollback_verified": not failures
                and self.current_root() == target_root,
                "failures": failures,
                "execution_status": "completed" if not failures else "blocked",
            },
        )
    ...
    def replay_session(self, receipt_path: str | Path | None = None) -> dict:
        ...
        if any(
            entry.get("event") in {"registry-sync", "lock", "install", "app-run"}
        ...
        if receipt_path is not None:
            try:
                ...
                if not verify_object_signature(unsigned, signature, self.trusted_keys)[
                    "verified"
                ]:
                    failures.append(
                        {
                            "path": str(receipt_path),
                            "reason": "source Genesis receipt signature is invalid",
                        }
                    )
                if source.get("format") != "BOGOS-Genesis-final-receipt-9.0":
                    failures.append(
                        {
                            "path": str(receipt_path),
                            "reason": "source is not a Genesis final receipt",
                        }
                    )
                ...
                if source.get("genesis_session_root") not in known_hashes:
                    failures.append(
                        {
                            "path": str(receipt_path),
                            "reason": "source Genesis session root is not in the ledger",
                        }
                    )
            except (OSError, KeyError, json.JSONDecodeError) as exc:
                failures.append(
                    {
                        "path": str(receipt_path),
                        "reason": f"invalid source Genesis receipt: {exc}",
                    }
                )
        ...
        for entry in entries:
            if entry["event"] == "trusted-boot":
                ...
                if entry["before_root_sha256"] != virtual_root:
                    failures.append(
                        {
                            "path": str(entry["sequence"]),
                            "reason": "write replay root mismatch",
                        }
                    )
                ...
                if not (self.objects_dir / entry["object_sha256"]).is_file():
                    failures.append(
                        {
                            "path": entry["object_sha256"],
                            "reason": "replay object missing",
                        }
                    )
                ...
                entry["event"] == "install"
                and entry.get("execution_status") == "completed"
            ):
                verification = self.workspace._verify_installed_package(
                    entry["package"]
                )
                if verification["execution_status"] != "completed":
                    failures.append(
                        {
                            "path": entry["package"],
                            "reason": "installed package failed session replay verification",
                        }
                    )
            elif entry["event"] == "app-run":
                process = entry.get("process_receipt", {})
                if self._stable_hash(process.get("proof_material", {})) != process.get(
                    "proof_sha256"
                ):
                    failures.append(
                        {
                            "path": entry.get("app", "app"),
                            "reason": "brokered process proof hash mismatch",
                        }
                    )
                for call in process.get("syscall_receipts", []):
                    ...
                    if self._stable_hash(evidence) != call.get("evidence_sha256"):
                        failures.append(
                            {
                                "path": entry.get("app", "app"),
                                "reason": "capability syscall evidence hash mismatch",
                            }
                        )
        if virtual_root != self.current_root():
            failures.append(
                {"path": "state", "reason": "replayed final workspace root mismatch"}
            )
        result = {
            "format": "BOGOS-Genesis-session-replay-receipt-9.0",
            "source_receipt": str(receipt_path) if receipt_path else None,
            "source_genesis_session_root": (
                source.get("genesis_session_root") if source else None
            ),
    ...
    def demo(self) -> dict:
        ...
        installs = [
            self.install("genesis-lib-1.0.0"),
            self.install("note-app-1.0.0"),
            self.install("calc-app-1.0.0"),
        ]
        kernel = BogKernel(self.workspace)
        ...
        forbidden_event = self.record(
            "forbidden-access",
            {
                ...
                "execution_status": (
                    "completed"
                    if forbidden["execution_status"] == "blocked"
                    else "blocked"
                ),
            },
        )
        installed = Path(
            read_store_index(self.workspace.bogos / "store")["packages"][
                "note-app-1.0.0"
            ]["install_dir"]
        )
        ...
        tamper_event = self.record(
            "tamper-rejection",
            {
                ...
                "execution_status": (
                    "completed"
                    if tampered["execution_status"] == "blocked"
                    else "blocked"
                ),
            },
        )
        ...
        checks = {
            ...
            "all_packages_signed": all(
                self._install_signature_ok(item) for item in installs
            ),
            "all_dependencies_verified": all(
                item["dependencies_verified"] for item in installs
            ),
            "all_apps_capability_brokered": note["execution_status"]
            == calc["execution_status"]
            == "completed",
            "all_state_changes_receipted": write["execution_status"] == "completed"
            and read["execution_status"] == "completed"
            and data.startswith(b"hello"),
            "forbidden_access_blocked": forbidden["execution_status"] == "blocked"
            and any(call["execution_status"] == "blocked" for call in calls),
            "tampered_package_blocked": tampered["execution_status"] == "blocked",
    ...
    def verify_ledger(self) -> dict:
        ...
        for expected, entry in enumerate(self.entries(), 1):
            ...
            if entry.get("sequence") != expected:
                failures.append(
                    {"path": str(expected), "reason": "ledger sequence mismatch"}
                )
            if entry.get("previous_hash") != previous:
                failures.append(
                    {"path": str(expected), "reason": "ledger previous hash mismatch"}
                )
            if self._stable_hash(payload) != receipt_hash:
                failures.append(
                    {"path": str(expected), "reason": "ledger receipt hash mismatch"}
                )
            if not verify_object_signature(unsigned, signature, self.trusted_keys)[
                "verified"
            ]:
                failures.append(
                    {
                        "path": str(expected),
                        "reason": "ledger signature verification failed",
                    }
                )
    ...
    def _save_manifest(self, files: dict) -> str:
        ...
        self._write_json(
            self.states_dir / f"{root}.json",
            {"format": "BOGFS-COW-state-9.0", "root_sha256": root, "files": files},
        )
    ...
    def _create_demo_packages(self) -> list[dict]:
        ...
        return [
            {"source": lib, "name": "genesis-lib", "version": "1.0.0"},
            {
                ...
                "name": "note-app",
                "version": "1.0.0",
                "dependencies": ["genesis-lib-1.0.0"],
            },
            {
                "source": calc,
                "name": "calc-app",
    ...
    def _write_demo_app(
        ...
        files = {
            "README.txt": f"BogOS Genesis signed {name}\n",
        ...
        manifest = {
            ...
            "apps": {
                name: {
                    ...
                    "expected_hashes": {
                        path: self._file_hash(root / path) for path in readable
                    },
                    ...
                    "capabilities": {
                        "read": [p for p in readable if p != "app.py"],
                        "write": [output],
                        "env": [],
    ...
    def _install_signature_ok(self, receipt: dict) -> bool:
        try:
            return receipt["package_install"]["store_receipt"][
    ...
    def _record_process(self, receipt: dict) -> dict:
        return self.record(
            "app-run",
            {
                ...
                "all_apps_capability_brokered": receipt.get("execution_status")
                == "completed",
    ...
    def _workspace_hash(self) -> str:
        return self._directory_hash(
            self.root, exclude={self.dir, self.workspace.bogos / "kernel"}
        )
    ...
    def _missing(self, name: str, required: bool) -> dict:
        ...
        return {
            "failures": failures,
            "execution_status": "blocked" if required else "completed",
        }
