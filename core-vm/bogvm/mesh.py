...
class BogMesh:
    ...
    def trust_peer(self, public_key: str | Path) -> dict:
        ...
        return {
            "format": "BOGMESH-peer-trust-receipt-11.0",
            "key_id": info["key_id"],
        ...
        claim = {
            **unsigned,
            ...
            "signature": sign_object(
                {**unsigned, "claim_id": claim_id}, self.genesis.private_key
            ),
        }
    ...
    def resolve(self, namespace: str) -> dict:
        ...
        for path in sorted(self.claims_dir.glob("*.json")):
            ...
            if claim.get("namespace") != namespace:
                continue
            failures = self._verify_claim(claim)
            (quarantined if failures else valid).append(
                {"claim": claim, "failures": failures}
            )
        groups: dict[str, list[dict]] = {}
        for item in valid:
            context_key = json.dumps(
                item["claim"]["context"], sort_keys=True, separators=(",", ":")
            )
        ...
        for context_key, claims in sorted(groups.items()):
            ...
            for claim in claims:
                by_value.setdefault(_stable_hash({"value": claim["value"]}), []).append(
                    claim
                )
            ranked = []
            ...
            if len(ranked) == 1:
                ...
                ranked[0]["authority"] > ranked[1]["authority"]
                or ranked[0]["pressure"] > ranked[1]["pressure"]
            ):
                policy = "winner"
                selected = ranked[0]
            else:
        ...
        state = {
            ...
            "outcomes": outcomes,
            "quarantined_claims": [
                item["claim"].get("claim_id") for item in quarantined
            ],
        }
        ...
        receipt = {
            **state,
            ...
            "conflict_detected": any(
                len(outcome["candidates"]) > 1 for outcome in outcomes
            ),
            "context_split": any(outcome["policy"] == "split" for outcome in outcomes)
            or len(outcomes) > 1,
            "execution_status": "completed",
        }
    ...
    def verify(self) -> dict:
        ...
        for path in sorted(self.claims_dir.glob("*.json")):
            failures.extend(
                {"path": str(path), "reason": item["reason"]}
                for item in self._verify_claim(json.loads(path.read_text()))
            )
        if self.state_path.is_file():
            ...
            if root != _stable_hash(state):
                failures.append(
                    {"path": str(self.state_path), "reason": "mesh state root mismatch"}
                )
        return {
    ...
    def _verify_claim(self, claim: dict) -> list[dict]:
        ...
        if claim.get("format") != CLAIM_FORMAT or claim_id != _stable_hash(material):
            failures.append(
                {"path": str(claim_id), "reason": "mesh claim format or hash mismatch"}
            )
        ...
        if not verify_object_signature(unsigned, signature, trusted)["verified"]:
            failures.append(
                {"path": str(claim_id), "reason": "mesh claim signature is not trusted"}
            )
        if not isinstance(claim.get("support"), int) or claim.get("support", 0) < 1:
            failures.append(
                {
                    "path": str(claim_id),
                    "reason": "mesh support must be a positive integer",
                }
            )
        if not claim.get("capability_scope"):
            failures.append(
                {"path": str(claim_id), "reason": "mesh claim has no capability scope"}
            )
        return failures
