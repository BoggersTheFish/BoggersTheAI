...
def softmax(logits):
    ...
    m = max(logits)
    exp_vals = [math.exp(l - m) for l in logits]
...
def print_receipt(title, inputs, soft_out, tens_out):
    ...
    print(f" -> Sum: {round(sum(soft_out), 2)} | Status: {'Diluted' if len(inputs) > 2 else 'Stable'}")
...
def run_demo():
    ...
    print("   It treats 10 undeniable facts as 'uncertainty' because it forces a zero-sum choice.")
